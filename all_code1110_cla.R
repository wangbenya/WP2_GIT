...
reclass_low <- function(df, i) {
  df[, "DON"][df[, "DON"] <= i] <- "Low"
  df[, "DON"][df[, "DON"] != "Low"] <- "else"
  df[, "DON"] <- factor(df[, "DON"], levels = c("Low", "else"))
  return(df)
}

reclass_high <- function(df, j) {
  df[, "DON"][df[, "DON"] >= j] <- "High"
  df[, "DON"][df[, "DON"] != "High"] <- "else"
  df[, "DON"] <- factor(df[, "DON"], levels = c("High", "else"))
  return(df)
}


reclass_medium<- function(df, i,j) {
  df[, "DON"][(df[, "DON"] >= i) & (df[, "DON"] <= j)] <- "Medium"
  df[, "DON"][df[, "DON"] != "Medium"] <- "else"
  df[, "DON"] <- factor(df[, "DON"], levels = c("Medium", "else"))
  return(df)
}
...
TN_GW4<-read.csv("~/WP2_GIT/TN_GW4.csv",header = T)
 ...
 # WP2Test$dep_cat<-WP2Test$Catchment*WP2Test$GW_depth
  ...
  WP2Train_low<-reclass_low(WP2Train,a1)
  WP2Test_low<-reclass_low(WP2Test,a1)
  
  WP2Train_medium<-reclass_medium(WP2Train,a1,a2)
  WP2Test_medium<-reclass_medium(WP2Test,a1,a2)
